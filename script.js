  ...
  for (let i = 1; i < itensList.length; i += 1) {
    if (itensList[i].classList.contains('selected')) {
      if (itensList[i].classList.contains('completed')) {
        const upPosition = itensList[i].innerHTML;
        const downPosition = itensList[i - 1].innerHTML;
...
function moveDown() {
  ...
  for (let i = itensList.length - 2; i >= 0; i -= 1) {
    if (itensList[i].classList.contains('selected')) {
      if (itensList[i].classList.contains('completed')) {
        const downPosition = itensList[i].innerHTML;
        const upPosition = itensList[i + 1].innerHTML;
